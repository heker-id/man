...
h={"Host":"m.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":"Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.105 Mobile Safari/537.36"}
...
ips=None
try:
	b=requests.get("https://hilman-toolkit2087.000webhostapp.com/ip.php").text.strip()
	ips=requests.get("https://ipapi.com/ip_api.php?ip="+b,headers={"Referer":"https://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":"MozillaMozilla/5.0 (iPhone; CPU iPhone OS 12_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.1 Mobile/15E148 Safari/604.1"}).json()["country_name"].lower()
except:
	ips=None
	
#if "pakistan" in ips:
#	ua="Mozilla/5.0 (Android 9; Mobile; rv:84.0) Gecko/84.0 Firefox/84.0"
...
def dumpfl():
	...
	if len(bs4.re.findall("logout",r)) !=0:
		...
		search(
			fl,cvds,
		"https://web.facebook.com/search/people/?q="+s)
	else:
		try:
			os.remove(".cok")
		except:
